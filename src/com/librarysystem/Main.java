...
public class Main {
    public static void main(String[] args) {
        ...
        Command addAudioBook = new AddAudioBookCommand();
        Command addEBook = new AddEBookCommand();
        Command addPhysicalBook = new AddPhysicalBookCommand();
        
        Book physicalBook = libraryFactory.createPhysicalBook(0, "Physical Book Title", "Author A", 300, true);
        Book physicalBook2 = libraryFactory.createPhysicalBook(1, "Physical Book2 Title", "Author B", 228, false);
        ...
        addPhysicalBook.execute(physicalBook);
        addPhysicalBook.execute(physicalBook2);
        addEBook.execute(eBook);
        addAudioBook.execute(audioBook);
...
class AddPhysicalBookCommand implements Command {
    public void execute(Book book) {
        BookManager.getInstance().addBook(book);
    }
}

class AddEBookCommand implements Command {
    public void execute(Book book) {
        BookManager.getInstance().addBook(book);
    }
}

class AddAudioBookCommand implements Command {
    public void execute(Book book) {
        BookManager.getInstance().addBook(book); // Добавление книги
    }
}